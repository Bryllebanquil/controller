...
export function ProcessManager({ agentId, isConnected }: ProcessManagerProps) {
  ...
  const [filteredProcesses, setFilteredProcesses] = useState([] as Process[]);
  const [loading, setLoading] = useState(false);
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedProcess, setSelectedProcess] = useState(null as Process | null);
  const [sortField, setSortField] = useState("cpu" as keyof Process);
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex gap-2">
              ...
              {React.createElement(Button, {
                ...
                variant: "outline",
                size: "default"
              }, React.createElement(RefreshCw, { className: `h-4 w-4 mr-2 ${loading ? "animate-spin" : ""}` }), "Refresh")}
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            <AlertDialog>
              <AlertDialogTrigger asChild>
                ...
                }, React.createElement(Shield, { className: "h-4 w-4" }), "Kill Task Manager")}
              </AlertDialogTrigger>
              <AlertDialogContent>
                <AlertDialogHeader>
            ...
            <div className="flex items-center gap-2">
              ...
              {React.createElement(Button, {
                ...
                disabled: !agentId || !isConnected
              }, React.createElement(Clock, { className: "h-4 w-4 mr-1" }), autoRefresh ? "ON" : "OFF")}
              {autoRefresh && (
                <Select
                ...
                <TableRow>
                  <TableHead>
                    {React.createElement(Button, {
                      ...
                      sortDirection === "asc" ? React.createElement(SortAsc, { className: "ml-1 h-3 w-3" }) : React.createElement(SortDesc, { className: "ml-1 h-3 w-3" })
                    ))}
                  </TableHead>
                  <TableHead>
                    {React.createElement(Button, {
                  ...
                  <TableHead>
                    {React.createElement(Button, {
                      ...
                      className: "h-8 p-0 font-medium"
                    }, React.createElement(Cpu, { className: "mr-1 h-3 w-3" }), "CPU %", sortField === "cpu" && (
                      sortDirection === "asc" ? React.createElement(SortAsc, { className: "ml-1 h-3 w-3" }) : React.createElement(SortDesc, { className: "ml-1 h-3 w-3" })
                    ))}
                  </TableHead>
                  <TableHead>
                    {React.createElement(Button, {
                      ...
                      className: "h-8 p-0 font-medium"
                    }, React.createElement(MemoryStick, { className: "mr-1 h-3 w-3" }), "Memory", sortField === "memory" && (
                      sortDirection === "asc" ? React.createElement(SortAsc, { className: "ml-1 h-3 w-3" }) : React.createElement(SortDesc, { className: "ml-1 h-3 w-3" })
                    ))}
                  </TableHead>
